...
iOS(左)とmacOS(右)のUIイメージ。テキスト未読込時は「アプリアイコン（透過率50%）」のプレースホルダーを表示。テキスト読込後は、現在の文を中央に大きく表示し、前後の文を薄い色で表示。上下に流れるような垂直スクロールアニメーションで文章が切り替わります。大きな丸型再生ボタン（Tiger Gold → Accent Orange グラデーション）を配置。右下にコピーボタンを配置。入力方法はドラッグ＆ドロップ、ファイルピッカー、⌘V ペーストの3種類に対応。
